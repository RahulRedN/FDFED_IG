...
const ReviewCard = ({content}) => {
  ...
  return (
    <div className="bg-white shadow flex gap-x-3 w-[40vw] py-3 pr-3 pl-2 rounded">
          <div className="flex items-center justify-center border-r px-2">
            <Bookmark  className={`hover:cursor-pointer ${
              issave ? "fill-amber-500 text-amber-500 ease-out duration-300" : "text-gray-600"
            } `}
            onClick={()=>{setIssave((prev) => !prev)}} strokeWidth={1} size={30} />
          </div>
